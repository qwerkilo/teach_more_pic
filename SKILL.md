...
22 个组件，每个组件的 HTML/CSS/JS 代码和用法规则在 `components/NN-name.md` 中。此处只列索引。
...
| 内容场景 | 推荐组件 | 备选 |
|---|---|---|
| 复杂概念分步拆解 | 折叠式分步详解 (#8) | 手风琴 FAQ |
| 多视角/多方案对比 | Tab 切换面板 (#9) 或 对比表 (#22) | — |
| 前后效果对比（图） | 图片对比滑块 (#10) | 标注式图片 (#14) |
| 事件时间序列（≤4） | CSS 时间线 (#3) | 编号列表 |
| 事件时间序列（5+） | 交互式时间线 (#11) | 状态链 (#15) |
| 关键统计数据 | 数据卡片网格 (#12) | + 数值滚动动画 (#16) |
| 人物引言/文献引用 | 引用/引文卡片 (#13) | 提示框 (#18) |
| 复杂图表/地图详解 | 标注式图片 (#14) | SVG 流程图 (#1) |
| 流程阶段/进度 | 状态链 (#15) 或 步骤指示器 (#20) | — |
| 重要提示/警告信息 | 提示框/告警条 (#18) | 引用卡片 (#13) |
| 关键词/分类展示 | 标签/徽章组 (#17) | — |
| 二维数据密度矩阵 | 热力图 (#19) | CSS 条形图 (#4) |
| 扩展阅读/术语详解 | 信息面板 (#21) | 折叠分步 (#8) |
| 参与者介绍 | 角色卡片 (#2) | 数据卡片 (#12) |
| 资金/逻辑流向 | SVG 流程图 (#1) | — |
| 多方案多维度对比 | 对比表增强版 (#22) | Tab 面板 (#9) |
| 单组数据对比 | CSS 条形图 (#4) | 数据卡片 (#12) |

选择原则：
- **第一幕**（设置矛盾）：通常用 1 个 SVG 流程图或时间线展示背景
- **第二幕**（危机爆发）：用交互式组件（Tab/时间线/标注图片）增强叙事张力
- **第三幕**（转折与遗产）：用对比表/数据卡片/状态链做收束总结
- **每幕最多 1 个视觉组件**，全课 3 个左右
...
Step 3: 写 HTML
  ...
  - 从 `components/NN-name.md` 复制所选组件的 HTML/CSS/JS 合并到模板中
  - **合并组件 CSS**：从各 components/NN-name.md 中将 CSS 代码块合并到课程 `<style>` 中。合并规则：
...
| # | 反模式 | 为什么 | 替代做法 |
|---|---|---|---|
| 1 | 课程中同时使用超过 4 种组件 | 视觉密度过高 | 每课选 2-3 个 |
