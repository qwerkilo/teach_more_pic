...
- 每课按内容自然选用合适的组件，每个 h2 章节至少 1 个视觉组件；**每课最少 6 个组件**（含标签组 #17），不设上限
- 新增组件：先在 `components/` 下创建文件，再更新此索引表
- 颜色语义全局统一：蓝=正常，橙=触发，红=崩溃，绿=救助
- **每课必须包含标签组 #17**，放在课程结尾（测验之前），使用 `#` 前缀（如 `# 宏观经济`、`# 次贷危机`）作为章节关键词总结
...
| # | 反模式 | 为什么 | 替代做法 |
|---|---|---|---|
| 1 | 组件数量少于 6 个 / 缺少标签组 #17 / 连续 4 段无视觉停顿 | 视觉密度不足，读者疲劳 | 每课最少 6 个组件（含标签组 #17），每 1-2 段插一个视觉元素 |
